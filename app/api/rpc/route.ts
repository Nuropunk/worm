...
import { WormHistory } from "@/components/types";
...
export const GET = async () => {
    ...
    const calculateNextPosition = (direction: string, p_wP: WormHistory) => {
    ...
    // Initialize the worm's movement history
    const wormHistory: WormHistory[] = [];
    ...
    for (const transactionGroup of transactionGroups) {
        const prevWormPosition = wormHistory[wormHistory.length - 1];
        ...
        const nextPosition = calculateNextPosition(direction, prevWormPosition);

        const wormHistoryItem: WormHistory = {
            x: nextPosition.x,
            y: nextPosition.y,
            direction,
            affectedTransactions,
        ...
        wormHistory.push(wormHistoryItem);
    }

    return new Response(JSON.stringify(wormHistory), {
        status: 200,
